<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard>
        <VCardTitle
          >{{ $t("project_base.your_projects") }}
          <VSpacer />
          <VTextField
            append-icon="mdi-magnify"
            :label="$t('project_base.global_search')"
            single-line
            hide-details
            v-model="search"
            ref="globalSearch"
            ...
            <tr :class="$style.filters">
              <td v-for="header in headers" :key="header.value">
                <VTextField
                  ...
                  :placeholder="`${$t('common.search')} ${header.text}`"
                  type="text"
